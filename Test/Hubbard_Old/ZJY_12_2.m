...
for epoch_index = 1:1:NumOfEpoch
    ...
    for time_index_mother = 2:1:TempSlice
        ...
        G_propa = Get_G_L2(1,TempSlice/2,NumOfVertexs,Sigma,D_Tau,lambda,TempSlice,K,T_hop,Miu,Uene);
        mea_result_gb2(end+1) = mean(diag(G_propa));
...
sample = mea_result_gb2;
[mean_gb2,svar_gb2] = Bin(sample*Beta);
fprintf("Mean=%f\t svar=%f\t ratio=%f\n",mean_gb2,svar_gb2,abs(svar_gb2/mean_gb2));
