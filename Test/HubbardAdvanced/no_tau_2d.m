           ...
           %% 铁磁图
            ...
            for site_i = 1:1:NumOfVertexs
                for site_j = 1:1:NumOfVertexs
                    ...
                    delta_x = min([abs(xi-xj),NumInEdge - abs(xi-xj)])+1;
                    delta_y = min([abs(yi-yj),NumInEdge - abs(yi-yj)])+1;
                    

                    S = green_up_c(site_i,site_i) * green_up_c(site_j,site_j) + green_up_c(site_i,site_j) * green_up(site_i,site_j) + ...
                    ...
                    AFM_fig(delta_x,delta_y) = AFM_fig(delta_x,delta_y) + S;
                    count_AFM(delta_x,delta_y) = count_AFM(delta_x,delta_y) + 1;
                end
            end
...
%% 处理AFM图
AFM_fig = real(AFM_fig)./count_AFM;
AFM_fig(1,1) = 0.0;%仅为了画图方便
pcolor(AFM_fig)
title(['U=',num2str(Uene),'   ','\beta=',num2str(Beta)])
% %% 处理Green函数
%等时部分
