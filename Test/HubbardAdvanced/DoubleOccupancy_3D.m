...
T_hop = 1.0;
NumInEdge = 5;
NumOfVertexs = NumInEdge^3;%维度，如果2维的话，SNN的表达式也许要改变
K = Get_K_3d(NumInEdge);%维度
Uene = 4;
Beta = 1;
D_Tau = 0.05;
NumOfWarm = 0;
NumOfEpoch = 50;
N_wrap = 5;%暂时地，我们要求TempSlice是N_wrap的整数倍
%% 与任务相关的新定义
px = pi;
py = pi;
pz = pi;
...
for epoch_index = 1:1:NumOfEpoch
    ...
    for time_index_mother = 2:1:TempSlice
        ...
        energy_k = 0.0;
        %% 测量可观测量
        if (mod(abs(time_index-time_choice),5)==0)
            %% 双占据
            for site_index_auxi = 1:1:NumOfVertexs
                mea_result_auxi(end+1) = green_down_c(site_index_auxi,site_index_auxi)*green_up_c(site_index_auxi,site_index_auxi);
            end
            mea_result_DD(end+1) = mean(diag(green_down_c).*diag(green_up_c));
            %% 能量
            ...
            mea_result_E(end+1) = energy_k/(NumOfVertexs);
           %% 最近邻S
            SNN = 0.0;
            a = 0.0;
            for site_i = 1:1:NumOfVertexs
                for site_j = 1:1:NumOfVertexs
                    if K(site_i,site_j) ~= 0
                        ...
                        SNN = SNN + a/(4*6);%4来自自旋，6来自最近邻，如果2d就要变成4
                    end
                end
            end
            SNN = real(SNN)/NumOfVertexs;
            mea_result_SNN(end+1) = SNN ;
           %% 结构因子
            S = 0.0;
            px = pi;
            py = pi;
