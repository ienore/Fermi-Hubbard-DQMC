...
for epoch_index = 1:1:NumOfEpoch
    ...
    if mod(zjy_index,8) == 0 && mod(epoch_index,NumOfEpoch/1000)==0
        fprintf("U = %f\tD_Tau = %f,MC_Ratio = %f\n",Uene,D_Tau,epoch_index/NumOfEpoch);
    end
...
%            end
           %% 测量G(beta/2)
            mea_result_Gb2_mat(:,:,count_mea) =  Get_G_L2_new(1,TempSlice/2)+ Get_G_L2_new(-1,TempSlice/2);
...
ZJY_result_mean(end+1) = mean_sample;
ZJY_result_svar(end+1) = svar_sample;
ZJY_U(end+1) = Uene;
end

errorbar(ZJY_U,ZJY_result_mean,ZJY_result_svar);
