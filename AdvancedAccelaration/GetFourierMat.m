...
green_p_c = zeros(2*NumInEdge+1);
for px_index = 1:1:2*NumInEdge+1
    for py_index = 1:1:2*NumInEdge+1
        delta_p = pi/(NumInEdge);
        ...
        green_p_c(px_index,py_index) =2.0 -  real(temp_sum) / (1*NumOfVertexs);%% Why ?
    end
end
