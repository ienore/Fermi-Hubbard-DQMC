function  [return_mean,return_svar] = Sub_Par(zjy_index,NumInEdge,NumOfWarm,NumOfEpoch,K,TempSlice,NumOfVertexs,Miu,Uene,D_Tau,lambda,T_hop)
%fprintf("FINAL_RUN:%d\n",FINAL_RUN);
...
mea_result_green_p_up_mat = zeros([NumInEdge,NumInEdge,NumOfEpoch*2]);
count = 1.0;
    ...
    for time_index_mother = 2:1:TempSlice
        ...
        for px_index = 1:1:NumInEdge
            for py_index = 1:1:NumInEdge
               px = px_index * (2*pi/NumInEdge);
               py = py_index * (2*pi/NumInEdge);
               FT_Mat = GetFourierMat(px,py,NumOfVertexs,NumInEdge);
               mea_result_green_p_up_mat(px_index,py_index,count) = ...
                   sum(green_up_c*FT_Mat,'all')/NumOfVertexs;
            end
        end 
        sum_measure = 0.0;
